...
import subtasksFakerFactory from "./subtasksFakerFactory";
...
const SubtasksFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = subtasksFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("subtasks").create(elem));
...
export default connect(mapState, mapDispatch)(SubtasksFakerDialogComponent);
