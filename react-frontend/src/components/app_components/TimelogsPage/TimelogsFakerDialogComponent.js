...
import timelogsFakerFactory from "./timelogsFakerFactory";
...
const TimelogsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = timelogsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("timelogs").create(elem));
...
export default connect(mapState, mapDispatch)(TimelogsFakerDialogComponent);
