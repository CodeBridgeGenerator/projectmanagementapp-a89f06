...
const ProjectsCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            clientID: _entity?.clientID,projectTitle: _entity?.projectTitle,description: _entity?.description,startDate: _entity?.startDate,endDate: _entity?.endDate,status: _entity?.status,
